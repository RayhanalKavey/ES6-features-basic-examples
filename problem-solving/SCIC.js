...
//---------------
//my try
//Find the students who got chance to the scic
const result = students.filter(
  (student) => student.avg >= 50 && student.fiftyPercent === true
);
// console.log(result);
result.forEach((seclectedStu) => {
  const { name, email } = seclectedStu;
  console.log(`Seleced studens name is ${name}, and email is${email} `);
});
//---------------
